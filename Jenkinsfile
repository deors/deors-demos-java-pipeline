            ...
            steps {
                ...
                container('aks') {
                    withCredentials([
                        ...
                        sh "kubectl expose pod ${TEST_CONTAINER_NAME} --port=${APP_LISTENING_PORT} --target-port=${APP_LISTENING_PORT}"
                        sh "kubectl expose pod ${TEST_CONTAINER_NAME} --port=${APP_JACOCO_PORT} --target-port=${APP_JACOCO_PORT} --name=${TEST_CONTAINER_NAME}-jacoco"
                    }
                }
            }
        }
